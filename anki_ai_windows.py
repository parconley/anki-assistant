...
def play_mp3_windows(filepath, speed=0.8):
    """
    Play MP3 file on Windows using pygame with speed control.
    """
    try:
        # Load and play the audio file
        pygame.mixer.music.load(filepath)
        
        # Pygame doesn't support direct speed control, so we use frequency adjustment
        # Lower frequency = slower playback
        freq = int(44100 * speed)  # Adjust sample rate for speed
        pygame.mixer.quit()
        ...
        log.error(f"Error playing MP3: {e}")
...
def tts(text):
    ...
    if cache_path.exists():
        # Play cached audio at 80% speed
        play_mp3_windows(str(cache_path), speed=0.8)
    else:
        # Use Windows TTS engine for immediate playback
        speakable_text = make_latex_speakable(text)
        tts_engine.say(speakable_text)
        tts_engine.runAndWait()
